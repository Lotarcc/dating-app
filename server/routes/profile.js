module.exports = (app, pool, upload, fs, path, bcrypt) => {
    ...
    app.post('/api/profile/setprofilepic', upload.single('file'), async (request, response) => {
        const session = request.session
        const picture = `${process.env.REACT_APP_FQDN}/images/` + request.file.filename
        if (session.userid) {
            ...
            try {
                ...
                if (profilePic.rows.length === 0) {
                    ...
                    const oldImage = path.resolve(__dirname, '../images') + oldImageData.replace(`${process.env.REACT_APP_FQDN}/images`, '')
    ...
    app.delete('/api/profile/deletepicture/:id', async (request, response) => {
        ...
        if (session.userid) {
            ...
            if (oldImageData !== `${process.env.REACT_APP_FQDN}/images/default_profilepic.jpeg`) {
                const oldImage = path.resolve(__dirname, '../images') + oldImageData.replace(`${process.env.REACT_APP_FQDN}/images`, '');
